...
parser.add_argument("--saved_dir", default='saved_traffic', type=str,
                        help="saved dir.")
...
if __name__ == "__main__":
	...
	systemWifi.extend([i%systemCount+1 for i in range(nCells-len(systemWifi))])
	...
	args.saved_dir = os.path.join(args.saved_dir, "outputs-"+str(systemCount-1))

	record_prefix = "-local_epochs-"+str(args.local_epochs)+ \
					"-batch_size-"+str(args.batch_size)+ \
					"-error_rate-"+str(args.error_rate)+ \
					"-nActivePerCell-"+str(args.nActivePerCell)+ \
	...
	if args.mpi and systemId==systemServer:
