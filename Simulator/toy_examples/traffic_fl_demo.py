...
def main():
    ...
    exp_results = []
    queue_len = 5
    test_mse_deque = deque([0.5] * queue_len, maxlen=queue_len)
    for epoch in range(1, args.epochs + 1):
        # Training the model for one epoch and get the results
        train_loss, d_link, u_link, consumed_energy, used_time, goodput, packet_loss = train(args, model_p,
                                                                                             criterion, device,
                                                                                             train_loader,
                                                                                             optimizer, epoch)
        ...
        if args.stop & (np.mean(test_mse_deque) < args.condition):
            break
    ...
    if args.save_model:
        torch.save(model_p.state_dict(), "traffic_lstm.pt")
